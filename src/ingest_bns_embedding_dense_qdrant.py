...
# Saving embeddings in Qdrant vector database
try:
    vectordb = QdrantVectorStore.from_documents(
        ...
        force_recreate=True, # This ensures clean collection creation
        batch_size=64,      # send 64 vectors per request
        prefer_grpc=True,
    )
    logger.info(f"Successfully saved embeddings to Qdrant collection: {collection_name}")
except Exception as e:
    logger.error(f"Error saving to Qdrant database: {str(e)}")
    raise